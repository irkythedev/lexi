...
import { Panel, Row, Tag, GhostButton, Segmented } from '../components/ui/primitives.tsx';
import SpeakButton from '../components/SpeakButton.tsx';
import PaginationBar from '../components/PaginationBar.tsx';
import { usePagination } from '../lib/pagination.ts';
import AiHistorySection from '../components/AiHistorySection.tsx';
...
export default function ErrorsView() {
  ...
  const [tab, setTab] = useState<'errors' | 'ai'>('errors');
  ...
  return (
    <div className="mx-auto max-w-[var(--max-grid)] px-[var(--pad-x)] pb-28 pt-4">
      <div className="mb-3 flex items-end justify-between">
        ...
        {tab === 'errors' && unitErrors.length > 0 && <GhostButton onClick={async () => { await clearResolvedErrors(); load(); toast(t('errorsClearedToast', locale), 'success', 'check'); }}>{t('errorsClear', locale)}</GhostButton>}
      </div>
      ...
      <div className="mb-4">
        <Segmented
          options={[
            { value: 'errors', label: t('errorsTitle', locale) },
            { value: 'ai', label: t('aiNotesTitle', locale) },
          ]}
          value={tab}
          onChange={setTab}
        />
      ...
      {tab === 'ai' ? (
        ...
        : <Panel>
          {pagedErrors.map((e) => {
            ...
            // 标签以错题自身 kind 为准（词形变换/微写作错题无 studyItem 也能正确显示）
            const meta = KIND_META[e.kind] ?? KIND_META.vocab;
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <Tag kind={e.kind}>{meta.label[locale]}</Tag>
                      {label && <span className="truncate text-[calc(15px*var(--type-scale))] font-semibold text-[var(--color-text)]">{label}</span>}
