  ...
  return (
    <div className="mx-auto max-w-[var(--max-read)] px-[var(--pad-x)] py-4">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-[clamp(20px,5vw,28px)] font-bold tracking-[-0.02em]"><Sparkles size={22} style={{ color: 'var(--color-accent)' }} /> AI 学习助手</h2>
        {cfg && <button onClick={() => setView('settings')} className="press flex items-center gap-1 text-[13px] text-[var(--color-text-2)]"><Settings2 size={15} /> 配置</button>}
      </div>
      ...
      {!unit && view === 'chat' && <div className="mt-3 rounded-[var(--radius-card)] border border-[var(--color-hairline)] p-3 text-[13px] text-[var(--color-text-2)]">提示：未选择教材单元时，AI 缺少页面上下文，批改与出题将基于你手动输入的内容。</div>}
    </div>
  );
}
...
function ConsentView({ onAgree }: { onAgree: () => void }) {
  ...
  return (
    <div className="rounded-[var(--radius-hero)] border p-6 shadow-[var(--shadow-panel)]" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-hairline)' }}>
      <div className="flex items-center gap-2 text-[15px] font-bold text-[var(--color-text)]"><AlertCircle size={18} style={{ color: 'var(--color-accent)' }} /> 使用须知</div>
      <ol className="mt-4 space-y-3">
        {TERMS.map((x, i) => <li key={i} className="text-[13.5px] leading-relaxed text-[var(--color-text-body)]"><span className="font-semibold text-[var(--color-text)]">{i + 1}. {x.t}：</span>{x.b}</li>)}
      </ol>
      <button onClick={onAgree} className="press mt-5 w-full rounded-full bg-[var(--color-accent)] py-3 text-[15px] font-semibold text-white">我已阅读并同意，继续配置 →</button>
    </div>
  );
}
...
function SettingsViewInline({ onSaved, initial }: { onSaved: (c: AiConfig) => void; initial: AiConfig | null }) {
  ...
  return (
    <div className="rounded-[var(--radius-hero)] border p-5 shadow-[var(--shadow-panel)]" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-hairline)' }}>
      <div className="text-[13px] font-semibold text-[var(--color-text-2)]">① 选择服务商（仅限大陆可用）</div>
      <div className="mt-2 flex flex-wrap gap-2">
        {AI_PROVIDERS.map((p) => <button key={p.id} onClick={() => onProvider(p.id)} className="press rounded-full border px-3.5 py-1.5 text-[13px] font-medium" style={{ borderColor: provider === p.id ? 'var(--color-accent)' : 'var(--color-hairline)', background: provider === p.id ? 'color-mix(in srgb, var(--color-accent) 10%, transparent)' : 'var(--color-surface)', color: provider === p.id ? 'var(--color-accent)' : 'var(--color-text-2)' }}>{p.name}</button>)}
      </div>
      {currentProvider?.note && <p className="mt-1.5 text-[12px] text-[var(--color-trap)]">{currentProvider.note}</p>}

      <div className="mt-4 text-[13px] font-semibold text-[var(--color-text-2)]">② API Key</div>
      <div className="relative mt-2">
        <input type={showKey ? 'text' : 'password'} value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-..." className="w-full rounded-[12px] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] py-2.5 pl-4 pr-11 text-[15px] outline-none focus:border-[var(--color-accent)]" />
      ...
      <div className="mt-4 text-[13px] font-semibold text-[var(--color-text-2)]">③ Base URL</div>
      <input value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://api.deepseek.com 或完整端点" className="mt-2 w-full rounded-[12px] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] px-4 py-2.5 text-[15px] outline-none focus:border-[var(--color-accent)]" />

      <div className="mt-4 text-[13px] font-semibold text-[var(--color-text-2)]">④ 模型</div>
      {liveModels.length > 0 ? (
        <select value={model} onChange={(e) => setModel(e.target.value)} className="mt-2 w-full rounded-[12px] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] px-4 py-2.5 text-[15px] outline-none focus:border-[var(--color-accent)]">
          {liveModels.map((m) => <option key={m} value={m}>{m}</option>)}
        </select>
      ) : (
        <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="或手动输入模型名，如 deepseek-chat" className="mt-2 w-full rounded-[12px] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] px-4 py-2.5 text-[15px] outline-none focus:border-[var(--color-accent)]" />
      )}
      <p className="mt-1 text-[12px] text-[var(--color-text-2)]">测试连接成功后会自动列出可用模型，也可手动输入。</p>

      <div className="mt-4 flex items-center gap-2">
        <button onClick={runTest} disabled={testing} className="press flex items-center gap-1.5 rounded-full border border-[var(--color-hairline)] px-4 py-2 text-[13px] font-medium text-[var(--color-text-2)] disabled:opacity-50">{testing ? '测试中…' : '测试连接'}</button>
        {testMsg && <span className="text-[12px] text-[var(--color-vocab)]">{testMsg}</span>}
      </div>
      {error && <p className="mt-3 rounded-[var(--radius-card)] bg-[var(--color-trap-soft)] p-3 text-[12.5px] text-[var(--color-trap)]">{error}</p>}

      <div className="mt-5 flex items-center justify-between">
        <button onClick={clearAll} className="press text-[13px] text-[var(--color-text-2)]">清除所有 AI 数据</button>
        <button onClick={save} className="press rounded-full bg-[var(--color-accent)] px-6 py-2.5 text-[15px] font-semibold text-white">保存配置</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[var(--radius-hero)] border shadow-[var(--shadow-panel)]" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-hairline)' }}>
      <div className="flex gap-1.5 border-b border-[var(--color-hairline)] p-3">
        {CAPS.map((c) => <button key={c.id} onClick={() => { setCap(c.id); setMessages([]); }} className="press flex-1 rounded-full px-2 py-2 text-[12.5px] font-medium" style={{ background: cap === c.id ? 'var(--color-accent)' : 'var(--color-track)', color: cap === c.id ? '#fff' : 'var(--color-text-2)' }}>{c.title}</button>)}
      </div>
      <div ref={scrollRef} className="h-[52vh] space-y-3 overflow-y-auto p-4">
        {messages.length === 0 && <div className="text-[13px] text-[var(--color-text-2)]"><p className="font-semibold text-[var(--color-text-2)]">{CAPS.find((c) => c.id === cap)?.title}</p><p className="mt-1">{CAPS.find((c) => c.id === cap)?.desc}</p>
        ...
        <p className="text-[12px] text-[var(--color-text-2)]">AI 生成内容仅供参考，请以教材和老师讲解为准。对话不保存、关页即清。</p>
      </div>
      <div className="border-t border-[var(--color-hairline)] p-3">
        {error && <p className="mb-2 text-[12px] text-[var(--color-trap)]">{error}</p>}
        <div className="flex items-end gap-2">
          <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }} rows={1} placeholder={cap === 'roleplay' ? '用英语回复 AI…' : '输入内容…'} className="flex-1 resize-none rounded-[var(--radius-card)] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] p-3 text-[15px] outline-none focus:border-[var(--color-accent)]" />
...
function Bubble({ msg }: { msg: { role: 'user' | 'assistant'; content: string; data?: CorrectionResult | ExamPointResult } }) {
  if (msg.role === 'user') return <div className="flex justify-end"><div className="max-w-[85%] rounded-[var(--radius-card)] bg-[var(--color-accent)] px-4 py-2.5 text-[15px] text-white">{msg.content}</div></div>;
  if (msg.data && 'questions' in msg.data) return (
    <div className="rounded-[var(--radius-card)] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] p-4">
      <div className="text-[13px] font-semibold text-[var(--color-text-2)]">考点选择题</div>
      {msg.data.questions.map((q, qi) => (
        <div key={qi} className="mt-3"><p className="text-[15px] font-medium text-[var(--color-text)]">{qi + 1}. {q.stem}</p>
          <ol className="mt-1.5 space-y-1">{q.options.map((o, oi) => <li key={oi} className={`rounded-[10px] border px-3 py-1.5 text-[13px] ${oi === q.answer ? 'border-[var(--color-vocab-border)] bg-[var(--color-vocab-soft)]' : 'border-[var(--color-hairline)]'}`}>{String.fromCharCode(65 + oi)}. {o}</li>)}
          </ol><p className="mt-1.5 text-[12px] text-[var(--color-text-2)]">解析：{q.explanation}</p></div>
      ))}
    </div>);
  if (msg.data && 'isCorrect' in msg.data) { const d = msg.data; return (
    <div className="rounded-[var(--radius-card)] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] p-4">
      <div className="flex items-center justify-between"><span className="text-[13px] font-semibold text-[var(--color-text-2)]">批改结果</span><span className={`rounded-full px-2.5 py-0.5 text-[12px] font-bold ${d.isCorrect ? 'bg-[var(--color-vocab-soft)] text-[var(--color-vocab-deep)]' : 'bg-[var(--color-trap-soft)] text-[var(--color-trap-deep)]'}`}>搭配分 {d.examCollocationScore}</span></div>
      <p className="mt-2 text-[15px] text-[var(--color-text-body)]">{d.correctedSentence}</p>
      <p className="mt-1.5 text-[13px] leading-relaxed text-[var(--color-text-2)]">{d.grammarBreakdown}</p>
    </div>); }
  return <div className="whitespace-pre-wrap rounded-[var(--radius-card)] border border-[var(--color-hairline)] bg-[var(--color-surface-2)] p-4 text-[15px] leading-relaxed text-[var(--color-text-body)]">{msg.content}</div>;
}
