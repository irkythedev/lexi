...
export default function FloatingTTS() {
  ...
  return (
    <>
      {/* 收起态：小圆按钮，点击展开 */}
      {collapsed ? (
        <button onClick={() => setOpen(true)} className="press fixed right-4 top-[calc(50%-20px)] z-40 flex h-10 w-10 items-center justify-center rounded-full border-2 border-[var(--color-hairline)] bg-[var(--color-surface)] text-[var(--color-accent)] shadow-[var(--shadow-card)]" aria-label={t('ttsExpand', locale)}>
          <Play size={16} />
        </button>
      ) : (
        <div className="fixed right-4 top-[calc(50%-20px)] z-40 flex items-center gap-0.5 rounded-[var(--radius-md)] border-2 border-[var(--color-hairline)] bg-[var(--color-surface)] px-1 py-0.5 shadow-[var(--shadow-card)]">
