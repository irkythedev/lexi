...
export default function CollocationConnector({ onExit }: { onExit: () => void }) {
  ...
  return (
    <div className="mx-auto max-w-[var(--max-read)] px-[var(--pad-x)] py-4">
      ...
      <h2 className="text-[calc(20px*var(--type-scale))] font-bold tracking-[-0.01em]">搭配拼图</h2>
      <p className="mt-1 text-[calc(13px*var(--type-scale))] text-[var(--color-text-2)]">点击左侧词干，再点右侧正确搭配，组成完整结构。错误会自动记入错题本。</p>
      ...
      {done ? (
        <div className="mt-8 flex flex-col items-center rounded-[var(--radius-hero)] p-8 text-center" style={{ background: 'var(--color-vocab-soft)', border: '1px solid var(--color-vocab-border)' }}>
          ...
          <p className="mt-3 text-[calc(18px*var(--type-scale))] font-bold text-[var(--color-text)]">全部配对成功！</p>
          <button onClick={reset} className="press mt-4 rounded-[var(--radius-md)] bg-[var(--color-text)] px-5 py-2 text-[calc(15px*var(--type-scale))] font-semibold text-[var(--color-surface)]">再来一次</button>
        </div>
      ) : (
        <div className="mt-5 space-y-5">
          <div>
            <div className="mb-2 text-[calc(12px*var(--type-scale))] font-semibold tracking-wide text-[var(--color-text-2)]">词干 / 结构前半</div>
          ...
          <div>
            <div className="mb-2 text-[calc(12px*var(--type-scale))] font-semibold tracking-wide text-[var(--color-text-2)]">搭配 / 介词 / 形式</div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {rights.map((r) => (
