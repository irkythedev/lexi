    ...
    if (this.state.error) {
      return (
        <div className="mx-auto max-w-[var(--max-read)] px-[var(--pad-x)] py-16 text-center">
          ...
          <button onClick={() => this.setState({ error: null })} className="press mt-6 rounded-full bg-[var(--color-accent)] px-5 py-2.5 text-[calc(15px*var(--type-scale))] font-semibold text-white">
            重试
          </button>
          <button onClick={() => window.location.href = '/'} className="press mt-3 ml-3 rounded-full border border-[var(--color-hairline)] px-5 py-2.5 text-[calc(15px*var(--type-scale))] font-medium">
            回到首页
          </button>
        </div>
      );
